...
class PhotoDetailsFragment : Fragment() {
    ...
    private var mViewModel: PhotoDetailsViewModelFactory.PhotoDetailsViewModel? = null
    ...
    private val newFlexbox: FlexboxLayout
        get() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mViewModel = ViewModelProviders.of(this, factory).get(PhotoDetailsViewModelFactory.PhotoDetailsViewModel::class.java)
    }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        mViewModel!!.getPhotoDetails().observe(this, Observer<PhotoDetails> { this.showSuccess(it) })

        mViewModel!!.getPhotoSizes().observe(this, Observer{ x ->
            photoSizes.postValue(x)
            if (x.sizes.size.size > 0) {
                val width = x.sizes.size.get(x.sizes.size.size - 1).width
                val height = x.sizes.size.get(x.sizes.size.size - 1).height
                textViewWidth!!.text = resources.getQuantityString(R.plurals.x_pixels, width, width)
                textViewHeigth!!.text = resources.getQuantityString(R.plurals.x_pixels, height, height)
            }
        })
    ...
    private fun showSuccess(photoDetails: PhotoDetails) {
        ...
        usernameLayout!!.setOnClickListener { v ->
            val intent = UserActivity.getStartingIntent(activity, photoDetails.photo.owner.nsid,
        ...
        viewsNumber!!.text = resources.getQuantityString(R.plurals.x_views, views, views)

        val locationStr = photoDetails.photo.owner.location
        ...
        val tags = photoDetails.photo.tags.tag
        if (tags.isEmpty()) {
    ...
    private fun simpleBinPack(chips: List<Chip>) {
        val containers = ArrayList<ArrayList<Chip>>()
        val freeSpace = ArrayList<Int>()

        Collections.sort(chips) { x, y ->
