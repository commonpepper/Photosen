...
class MainActivity : AbstractNavActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        navigationView.menu.findItem(R.id.drawer_popular).isCheckable = true
        navigationView.menu.findItem(R.id.drawer_popular).isChecked = true
        ...
        val dateFormatApi = SimpleDateFormat("yyyy-MM-dd")
        val dateFormatTitle = SimpleDateFormat("dd.MM")
        val cal = Calendar.getInstance()
        cal.add(Calendar.DATE, -1)
        for (i in 1 until FRAGMENTS_NUM) {
            cal.add(Calendar.DATE, -1)
            photoListFragments[i] = PhotoListFragment.newInstance(dateFormatApi.format(cal.time))
    ...
    companion object {
        private val PREFERENCE_FIRST_LAUNCH = "MAIN_FIRST_LAUNCH"

        private val FRAGMENTS_NUM = 7
    }
}
